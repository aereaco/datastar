...
from .sse import SSE_HEADERS, StateEvent, ServerSentEventGenerator
...
class StateResponse(HTTPResponse):
    default_headers: dict[str, str] = SSE_HEADERS.copy()

    def __init__(
        self,
        content: StateEvent | Collection[StateEvent] | None = None,
...
async def state_respond(
    request: Request, *, status: int = 200, headers: Mapping[str, str] | None = None
) -> StateResponse:
    return await request.respond(StateResponse(status=status, headers=headers))
