# Architecture Decision Record: Nexus-UX SDK

## Summary

Nexus-UX has had a few helper tools in the past for different languages.  The SDK effort is to unify around the tooling needed for Hypermedia On Whatever your Like (HOWL) based UIs.  Although Nexus-UX the library can use any plugins, the default bundle includes robust Server Sent Event (SSE) base approach.  Most current languages and backend don't have great tooling around the style of delivering content to the frontend.
...
The core mechanics of Nexus-UX’s SSE support is

1. Data gets sent to browser as SSE events.
2. Data comes in via JSON from browser under a `state` namespace.
...
##### EventType
An enum of Nexus-UX supported events.  Will be a string over the wire.
Currently valid values are

| Event                     | Description                         |
|---------------------------|-------------------------------------|
| state-merge-fragments  | Merges HTML fragments into the DOM  |
| state-merge-signals    | Merges signals into the signals       |
| state-remove-fragments | Removes HTML fragments from the DOM |
| state-remove-signals   | Removes signals from the signals      |
| state-execute-script   | Executes JavaScript in the browser  |
...
An enum of Nexus-UX supported fragment merge modes.  Will be a string over the wire
...
* `mergeMode` (FragmentMergeMode) The mode to use when merging the fragment into the DOM.  If not provided the Nexus-UX client side ***will*** default to `morph`.
* `useViewTransition` Whether to use view transitions, if not provided the Nexus-UX client side ***will*** default to `false`.

#### Logic
When called the function ***must*** call `ServerSentEventGenerator.send` with the `state-merge-fragments` event type.
...
`selector` is a CSS selector that represents the fragments to be removed from the DOM.  The selector ***must*** be a valid CSS selector.  The Nexus-UX client side will use this selector to remove the fragment from the DOM.
...
#### Logic
1. When called the function ***must*** call `ServerSentEventGenerator.send` with the `state-remove-fragments` event type.
...
Data is a JavaScript object or JSON string that will be sent to the browser to update signals in the signals.  The data ***must*** evaluate to a valid JavaScript.  It will be converted to signals by the Nexus-UX client side.
...
#### Logic
When called the function ***must*** call `ServerSentEventGenerator.send` with the `state-merge-signals` event type.
...
`paths` is a list of strings that represent the signal paths to be removed from the signals.  The paths ***must*** be valid `.` delimited paths to signals within the signals.  The Nexus-UX client side will use these paths to remove the data from the signals.

#### Logic
When called the function ***must*** call `ServerSentEventGenerator.send` with the `state-remove-signals` event type.
...
#### Logic
When called the function ***must*** call `ServerSentEventGenerator.send` with the `state-execute-script` event type.
...
1. The function ***must*** parse the incoming HTTP request
   1. If the incoming method is `GET`, the function ***must*** parse the query string's `state` key and treat it as a URL encoded JSON string.
