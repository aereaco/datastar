(ns aereaco.nexus-ux.clojure.sdk-test.core
  (:require
...
(def options (set (keys str->state-opt)))

;; -----------------------------------------------------------------------------
;; Testing code: We want to send back event received as state signal values
;; -----------------------------------------------------------------------------
(defn merge-fragments! [sse event]
  (let [frags (get event "fragments")
        opts (-> event
                 (select-keys options)
                 (set/rename-keys str->state-opt))]
