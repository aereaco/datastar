...
var (
	// ErrNoPathsProvided is returned when no paths were provided for // for [state.RemoveSignals] call.
...
// ReadSignals extracts Nexus-UX signals from
...
func ReadSignals(r *http.Request, signals any) error {
	...
	if r.Method == "GET" {
		dsJSON := r.URL.Query().Get(StateKey)
		if dsJSON == "" {
			return nil
		} else {
			dsInput = []byte(dsJSON)
