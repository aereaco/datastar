...
import aereaco.nexus-ux.adapters.request.RequestAdapter;
...
import static aereaco.nexus-ux.Consts.STATE_KEY;
...
public class SignalReader {
    ...
    public static void readSignals(RequestAdapter requestAdapter, ConcurrentMap<String, Object> store) throws IOException {
        ...
        if ("GET".equalsIgnoreCase(requestAdapter.getMethod())) {
            // Handle GET requests by parsing the `state` query parameter
            data = requestAdapter.getParameter(STATE_KEY);
            if (data == null || data.isBlank()) {
                throw new IllegalArgumentException("Missing 'state' query parameter in GET request.");
            }
        } else {
